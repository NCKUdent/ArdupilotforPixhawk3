...
int32_t AP_PitchController::_custom_get_rate_out(float desired_rate, bool disable_integrator)
{
	uint32_t tnow = hal.scheduler->millis();
	uint32_t dt = tnow - _last_t;
	if (_last_t == 0 || dt > 1000) {
    ...
    if (!disable_integrator) {
		if (dt > 0) {
		    pitch_I_integrator += rate_error * delta_time;
			pitch_D_derivative = (rate_error - rate_error_prior) / delta_time;
			rate_error_prior = rate_error;
		} else {
		//pitch_I_integrator = 0;
		rate_error_prior = 0;
	    }
	} else {
		pitch_I_integrator = 0;
	...
	// path, but want a 1/speed^2 scaler applied to the rate error path. 
	// This is because acceleration scales with speed^2, but rate scales with speed.
	_last_out = (rate_error * inner_P) + (pitch_I_integrator * inner_I) + (pitch_D_derivative * inner_D);
	_last_out_deg = ToDeg(_last_out);
	
	// Convert to centi-degrees and constrain, beware for physical system constraints
	return constrain_float(_last_out_deg * 100, -1100, 1300);
}

int32_t AP_PitchController::custom_get_servo_out(int32_t angle_err, bool disable_integrator)
{
